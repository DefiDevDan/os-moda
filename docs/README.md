...
1. **[SPAWN-API.md](SPAWN-API.md)** — full **v1.2.7** public API reference. `POST /api/v1/spawn/:planId` + credentials, idempotency, structured errors, token lifecycle, spec-kit project discovery, **SSE async chat** (`chat-async` / `chat-stream` / `chat-history`), **managed agent restart** (`POST /agents/:agent/restart`), **bulletproof PAM auto-recovery** (transparent operational hardening). Live spec at <https://spawn.os.moda/api/v1/docs>.
