...
runtime** (claude-code + openclaw drivers, v0.2 of the gateway), Spawn API **v1.2.5** with
idempotency, structured errors, encrypted credential store, per-server dashboard config,
spec-kit project discovery, `runtimes[].supported_auth_types` (OAuth gating), and
**SSE-based async chat** with cursor-resumable streaming on the dashboard surface
(`chat-async` / `chat-stream` / `chat-history`).
...
| Public spawn API **v1.2.5** — idempotency, structured errors, token lifecycle, spec-kit discovery, OAuth/auth-type gating, **SSE async chat** (cursor-resumable, persistent NDJSON log) | spawn.os.moda v1 API | **Solid** |
