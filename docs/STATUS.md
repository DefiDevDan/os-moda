...
**Managed Agents runtime — prototype (2026-06-02, gateway v0.3.0).** A third runtime driver, `managed-agent` (`src/drivers/managed-agent.ts`), drives Anthropic's **Managed Agents** via the `ant` CLI. Unlike claude-code/openclaw (LOCAL, root on the box), a Managed Agent runs in **Anthropic's cloud sandbox** — so it's for the sandboxed/app/workload tier (untrusted tools, deploy-ai-agent, scale-out workers), **not** the tier-0 system agent. It maps the beta session-stream events to the SAME osModa event contract, so the relay + dashboard render it identically; healthCheck gates on the `ant` binary; api_key-only (Managed Agents = the API; OAuth-CLI tokens may lack API access). Pure event mapper unit-tested (6 tests; 34 gateway tests green). **Prototype status:** registered + builds + maps the contract; the beta stream-event field names are mapped permissively and need tightening against one real run; live use requires `ant` installed + an api_key with Managed Agents beta access. Selectable per-agent in the Engine tab (health-gated, so the swap can't break chat).
