    ...
    async healthCheck() {
        ...
        try {
            ...
            return {
                ...
                remediation: "Install: cd /opt/osmoda/packages/osmoda-gateway && npm install && " +
        ...
        const out = await new Promise((resolve) => {
            ...
            catch { } ; resolve({ code: 124, stdout, stderr }); }, 5000);
        });
        if (out.code !== 0) {
            return {
                available: false,
        ...
        if (major < 2) {
            return {
                ...
                remediation: "Upgrade: npm install -g @anthropic-ai/claude-code@^2.1.75 " +
        ...
        try {
            for await (const line of rl) {
                ...
                if (t === "system" && event.subtype === "init") {
                    ...
                    const msgId = event.message?.id || "msg-default";
...
function toolTargetHint(input) {
    ...
    if (input.path)
        return clip(input.path, 120);
    if (input.url)
        return clip(input.url, 120);
    if (input.pattern)
