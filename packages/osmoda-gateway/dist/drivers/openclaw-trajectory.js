 ...
 */
function extractRoundText(data) {
    if (!data || typeof data !== "object")
        return "";
    if (Array.isArray(data.assistantTexts) && data.assistantTexts.length) {
        return data.assistantTexts.filter((s) => typeof s === "string").join("\n");
    }
    ...
    for (const c of [data.message, data.assistant, data.output, data.content]) {
        for (const m of asArray(c)) {
            ...
            const joined = asArray(content).filter((b) => b && b.type === "text" && b.text).map((b) => b.text).join("");
            if (joined)
                return joined;
        }
    }
 ...
 */
function extractSnapshotTools(data) {
    const out = [];
    ...
    msgs.forEach((m, i) => {
        ...
        asArray(m.content).forEach((b, bi) => {
