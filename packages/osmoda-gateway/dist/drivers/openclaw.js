...
function writeAuthProfile(agentId, cred) {
    ...
    const next = prev.then(async () => {
        ...
        const credential = cred.type === "oauth"
 ...
 */
const agentRegisterLocks = new Map();
function ensureAgentRegistered(bin, agentId, workspace) {
...
export const openClawDriver = {
    ...
    async healthCheck() {
        ...
        if (!bin) {
            return {
                ...
                remediation: "Install: mkdir -p /opt/openclaw && cd /opt/openclaw && " +
        ...
        const { code, stdout, stderr } = await new Promise((resolve) => {
            ...
            catch { } ; resolve({ code: 124, stdout: so, stderr: se }); }, 5000);
        });
        if (code !== 0) {
            return {
                available: false,
        ...
        if (!hasAgent) {
            return {
                available: false,
                version,
                error: `Installed ${version || "openclaw"} does not expose the 'openclaw agent' subcommand ` +
                    `this driver invokes. Expected OpenClaw 2026.5+.`,
                remediation: "Upgrade: cd /opt/openclaw && npm install openclaw@latest, then restart osmoda-gateway.",
            };
        }
    ...
    async *startSession(opts) {
        ...
        try {
            await writeAuthProfile(opts.agent.id, opts.credential);
        }
        catch (e) {
            yield {
                type: "error",
        ...
        try {
            await ensureAgentRegistered(bin, opts.agent.id, cwd);
        }
        catch { /* best-effort */ }
        ...
        proc.stdout?.on("data", (d) => { stdoutText += d.toString(); });
        const extractText = (v) => {
            ...
            if (typeof v.output === "string" && v.output.trim())
                return v.output;
            if (typeof v.result === "string" && v.result.trim())
                return v.result;
            ...
            if (v.data) {
                const inner = extractText(v.data);
                if (inner)
                    return inner;
            }
            if (v.payload) {
                const inner = extractText(v.payload);
                if (inner)
                    return inner;
            }
            return undefined;
        ...
        let trajOffset = 0;
        try {
            trajOffset = fs.statSync(trajPath).size;
        }
        catch { /* new session, file not created yet */ }
        let trajBuf = "";
        ...
        if (trimmed) {
            try {
                ...
                for (const line of trimmed.split("\n")) {
                    ...
                    if (!s.startsWith("{") && !s.startsWith("["))
                        continue;
                    try {
                        const t = extractText(JSON.parse(s));
                        if (t)
                            replyText = t;
                    }
                    catch { /* skip */ }
            ...
            if (!replyText && code === 0 && !/^[\s{[]/.test(trimmed))
                replyText = trimmed;
        }
        if (replyText) {
        ...
        if (code !== 0 && !hasOutput && !opts.abortSignal?.aborted) {
            ...
            const errLine = (stderrText.match(/error="?([^"\n]+)"?/) || [])[1] ||
