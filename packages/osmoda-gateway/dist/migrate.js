...
import { loadAgentsFile, saveAgentsFile } from "./config.js";
...
export function runMigrationIfNeeded() {
        ...
        try {
            ...
            if (Array.isArray(raw.credentials)) {
                ...
                for (const c of raw.credentials) {
                    ...
                    const dupe = existingCreds.find((x) => x.label === c.label && x.provider === c.provider && x.type === c.type &&
    ...
    const runtimeDetected = gatewayUnit && /openclaw\b/i.test(gatewayUnit) ? "openclaw" : "claude-code";
    report.detected_runtime = runtimeDetected;
    // Pick a credential to default agents to.
    const creds = loadCredentials().credentials;
    const defaultCredId = openClawCredByAgent["osmoda"] ||
        claudeCredId ||
        creds[0]?.id ||
        null;
    // Build default agent profiles.
    const now = new Date().toISOString();
    const mkAgent = (id, displayName, model, channels, runtime, credentialId) => ({
        id,
