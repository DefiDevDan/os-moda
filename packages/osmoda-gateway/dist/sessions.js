...
const DEFAULT_PATH = process.env.OSMODA_SESSIONS_PATH ||
...
export class SessionStore {
    ...
    getOrCreate(userId, channel, agentId, runtime) {
        ...
        if (existing) {
            ...
            if (runtime && existing.runtime && existing.runtime !== runtime && existing.claudeSessionId) {
                // eslint-disable-next-line no-console
                console.log(`[sessions] runtime swap ${existing.runtime} → ${runtime} for key=${key}; ` +
            ...
            this.scheduleSave();
            // eslint-disable-next-line no-console
            console.log(`[sessions] resume key=${key} runtime=${existing.runtime || "?"} ` +
    ...
    updateClaudeSession(userId, channel, claudeSessionId, runtime) {
        const key = `${channel}:${userId}`;
        ...
        if (changed) {
            // eslint-disable-next-line no-console
            console.log(`[sessions] bind key=${key} runtime=${session.runtime || "?"} claude_session=${claudeSessionId}`);
        }
        this.scheduleSave();
    }
    /** Forget the claude session id (e.g. user asks to start over). */
    clearClaudeSession(userId, channel) {
        const key = `${channel}:${userId}`;
        const session = this.sessions.get(key);
        if (!session)
            return;
