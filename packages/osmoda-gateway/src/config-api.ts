...
export interface ConfigApiDeps {
  ...
  spendCheck?: (agent: AgentProfile) => {
    allowed: boolean; tokensUsed: number; usdUsed: number; tokenCap?: number; usdCap?: number; pct: number;
  };
}
  ...
  try {
    ...
    if (url.pathname === "/config/agents" && req.method === "PUT") {
      ...
      for (const a of body.agents as AgentProfile[]) {
        ...
        for (const capKey of ["dailyTokenCap", "dailyUsdCap"] as const) {
          const v = (a as any)[capKey];
