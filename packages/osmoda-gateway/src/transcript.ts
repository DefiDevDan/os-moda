...
const TRANSCRIPT_DIR =
  process.env.OSMODA_TRANSCRIPT_DIR ||
...
export type TranscriptInput = Omit<TranscriptEvent, "seq" | "ts"> & { ts?: string };
...
export class TranscriptStore {
  ...
  private fileFor(agentId: string, sessionKey: string): string {
  ...
  read(agentId: string, sessionKey: string, sinceSeq = 0): TranscriptEvent[] {
    try {
      ...
      return fs
        ...
        .map((l) => {
          try {
            return JSON.parse(l) as TranscriptEvent;
          } catch {
            return null;
          }
        })
        .filter((e): e is TranscriptEvent => !!e && typeof e.seq === "number" && e.seq > sinceSeq);
    } catch {
      return [];
    }
  }
  ...
  list(): Array<{ agentId: string; sessionKey: string; events: number; updated_at: string }> {
    const out: Array<{ agentId: string; sessionKey: string; events: number; updated_at: string }> = [];
    try {
   ...
   */
  buildRecap(agentId: string, sessionKey: string, maxChars = 6000): string | null {
    const evs = this.read(agentId, sessionKey);
    if (!evs.length) return null;
    const lines: string[] = [];
    for (const e of evs) {
      if (e.role === "user" && e.text) lines.push(`User: ${e.text}`);
      else if (e.role === "assistant" && e.text) lines.push(`You (osModa): ${e.text}`);
