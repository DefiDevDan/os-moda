...
- **Remember across conversations (persistent memory)**: the gateway auto-loads `/var/lib/osmoda/memory/MEMORY.md` plus today's and yesterday's `daily/YYYY-MM-DD.md` notes into your system prompt at the **start of every session** — this is the source of truth for what you remember between conversations. When you learn a durable fact, a user preference, or make a decision that should outlive this chat, **record it**: append a concise line to `MEMORY.md` (via `Write`/`Edit`, or the `memory_store` MCP tool), and put detailed time-stamped working notes in `memory/daily/<today>.md`. Keep `MEMORY.md` tight — facts and decisions, not transcripts. Your full conversation is separately and durably preserved by the gateway's canonical transcript (it survives restarts and even a runtime swap), so you do not need to re-state prior turns.
